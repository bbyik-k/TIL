...
[ECMA Specification](https://tc39.es/ecma262/) 공식 문서의 내용을 AI를 통해 분석 했을 때 "ECMAScript 사양은 primitive 값의 종류 및 속성을 정의하지만, 어디에 어떻게 저장되는지 또는 주소를 재사용하는지에 대해서는 전혀 언급하지 않았음." 이라는 의견을 도출하였다.

[MDN JavaScript의 타입과 자료구조](https://developer.mozilla.org/ko/docs/Web/JavaScript/Guide/Data_structures) 페이지에서도 마찬가지로 메모리 주소 재사용에 대한 언급은 되고 있지 않다.

스택과 힙에 대한 사용조차 공식 규정에 나와있지 않다.

JS는 C, C++과 같이 포인터를 사용하고, 메모리 주솟값을 반환하는 메서드도 존재하지 않는다.
